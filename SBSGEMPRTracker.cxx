...
SBSGEMPRTracker::SBSGEMPRTracker( const char* name, const char* desc, THaApparatus* app ):
...
SBSGEMPRTracker::~SBSGEMPRTracker(){
  //fTestTracks->Clear("C");
  //delete fTestTracks;
}


THaAnalysisObject::EStatus SBSGEMPRTracker::Init( const TDatime& date ){
  //assert( fCrateMap == 0 );
 
  // Why THaNonTrackingDetector::Init() here? THaNonTrackingDetector doesn't implement its own Init() method
  // Is that only because this class inherits THaNonTrackingDetector directly? Is there any advantage to this
...
Int_t SBSGEMPRTracker::ReadDatabase( const TDatime& date ){
  std::cout << "[Reading SBSGEMPRTracker database]" << std::endl;
  ...
  if( modules.empty()) {
    Error("", "[SBSGEMPRTracker::ReadDatabase] No modules defined");
  }
...
Int_t SBSGEMPRTracker::Begin( THaRunBase* run ) {
  for( auto& module: fModules ) {
    module->Begin(run);
  }
...
void SBSGEMPRTracker::Clear( Option_t *opt ){
...
Int_t SBSGEMPRTracker::Decode(const THaEvData& evdata ){
  //return 0;
  //std::cout << "[SBSGEMPRTracker::Decode], decoding all modules, event ID = " << evdata.GetEvNum() <<  "...";
  ...
  if( fUseTrigTime ){
    ...
    //    std::cout << "[SBSGEMPRTracker::Decode]: ntrigtdchits = " << ntrigtdchits << std::endl;
...
Int_t SBSGEMPRTracker::End( THaRunBase* run ){
...
void SBSGEMPRTracker::Print(const Option_t* opt) const {
...
void SBSGEMPRTracker::SetDebug( Int_t level ) {
...
Int_t SBSGEMPRTracker::DefineVariables( EMode mode ){
...
Int_t SBSGEMPRTracker::CoarseProcess( TClonesArray& tracks ){
  
  
  if( !fUseConstraint && !fPedestalMode ){
    //std::cout << "SBSGEMPRTracker::CoarseProcess...";
    ...
    //std::cout << "calling find_tracks coarseprocess..." << std::endl;
    find_tracks();
...
Int_t SBSGEMPRTracker::FineProcess( TClonesArray& tracks ){

  
  if( fUseConstraint && !fPedestalMode ){ //
    //std::cout << "SBSGEMPRTracker::FineProcess...";
    ...
    //std::cout << "calling find_tracks fineprocess..." << std::endl;
      find_tracks();
    }
    //std::cout << "done. found " << fNtracks_found << " tracks" << std::endl;
